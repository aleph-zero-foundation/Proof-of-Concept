...
def generate_dag_unit(dag, posets, creator_id, strategy, unit_to_name, forking, only_maximal_parents):
    n_processes = len(posets)
    if forking:
...
def initialize_posets(n_processes):
...
def simulate_with_checks(
    ...
    dag = DAG(n_processes)
    posets = initialize_posets(n_processes)
    ...
    while len(dag) < n_units:
        ...
        res = generate_dag_unit(dag, posets, creator_id, strategy, unit_to_name, forking = creator_id in forkers, only_maximal_parents = only_maximal_parents)
        if res is None:
